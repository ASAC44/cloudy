...
PODEX_APPROVE_PIN=5
PODEX_REJECT_PIN=6
PODEX_IDLE_ANIMATION_SECONDS=30
```
...
`PODEX_STATE_DIR/settings.json`. Brightness currently dims the rendered UI;
volume uses the native ALSA `Master` control through `amixer`. Connectivity,
reduced motion, and the idle-animation timeout are configurable from the same
touchscreen list.
