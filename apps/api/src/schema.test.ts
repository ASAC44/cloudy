...
test('message history imports are consented, leased, idempotent, and reversible', async () => {
  const migration = await readFile(new URL('supabase/migrations/20260722050000_message_history_controls.sql', root), 'utf8')
  const rollback = await readFile(new URL('supabase/rollback/20260722050000_message_history_controls.sql', root), 'utf8')
  assert.match(migration, /^begin;/)
  assert.match(migration, /add column learned_actions_enabled boolean not null default false/)
  assert.match(migration, /source_kind = 'imported_sent' and decision_case_id is null and provider_message_id_hash is not null/)
  assert.match(migration, /create unique index memory_message_examples_import_source/)
  assert.match(migration, /create or replace function public\.configure_memory_import/)
  assert.match(migration, /status = 'connected'[\s\S]*for share/)
  assert.match(migration, /create or replace function public\.claim_memory_import/)
  assert.match(migration, /for update skip locked limit 1/)
  assert.match(migration, /create or replace function public\.record_imported_message/)
  assert.match(migration, /for update;[\s\S]*on conflict \(owner_id, connection_id, provider_message_id_hash\)/)
  assert.match(migration, /grant execute on function public\.claim_memory_import\(integer\) to service_role/)
  assert.match(migration, /create or replace function public\.forget_memory_scope/)
  assert.match(migration, /pg_advisory_xact_lock/)
  assert.match(migration, /memory_forget_action_in_flight/)
  assert.match(migration, /'user\.rebuild'/)
  assert.match(rollback, /Refusing rollback while imported message history exists/)
  assert.match(migration, /commit;\s*$/)
  assert.match(rollback, /commit;\s*$/)
})
