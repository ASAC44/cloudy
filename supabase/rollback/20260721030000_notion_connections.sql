...
do $$
begin
  if exists (select 1 from public.connections where provider = 'notion')
    or exists (select 1 from public.connection_oauth_states where provider = 'notion')
    or exists (select 1 from public.agent_memories where provider = 'notion')
    or exists (select 1 from public.pods where screen_layout::text like '%app:notion%') then
    raise exception 'Refusing rollback while Notion data or screen assignments exist';
  end if;
end;
$$;
...
alter table public.connections add constraint connections_provider_check
  check (provider in ('github', 'gmail', 'google_calendar', 'vercel', 'telegram', 'linear', 'stripe', 'custom_mcp'));
...
alter table public.connection_oauth_states add constraint connection_oauth_states_provider_check
  check (provider in ('github', 'gmail', 'google_calendar'));
  ...
  return coalesce(cardinality(attachment_ids), 0) = coalesce((select count(distinct value) from unnest(attachment_ids) value), 0)
    and not exists (
      ...
      where attachment_id not in (
        'app:github', 'app:gmail', 'app:google_calendar', 'app:codex', 'app:vercel',
