...
alter table public.pods drop constraint if exists pods_valid_screen_layout;
...
set screen_layout = jsonb_build_object(
      'left', case when jsonb_array_length(screen_layout -> 'left') > 0 then jsonb_build_array((screen_layout -> 'left') -> 0) else '[]'::jsonb end,
      'right', case when jsonb_array_length(screen_layout -> 'right') > 0 then jsonb_build_array((screen_layout -> 'right') -> 0) else '[]'::jsonb end,
      'down', case when jsonb_array_length(screen_layout -> 'down') > 0 then jsonb_build_array((screen_layout -> 'down') -> 0) else '[]'::jsonb end
    ),
    screen_layout_revision = screen_layout_revision + 1
where jsonb_array_length(screen_layout -> 'left') > 1
   or jsonb_array_length(screen_layout -> 'right') > 1
   or jsonb_array_length(screen_layout -> 'down') > 1;
